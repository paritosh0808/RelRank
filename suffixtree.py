...
class SuffixTree(object): 
	...
	if bool(node.out): 	
		...
		print 'Match found in the following tale at index', node.index
...
def first_occurence(qstring, fn_list, stree):
	for i in range(1,314):                
		...
		if result is 1:
			print "\nMatch found in the following tale at index", index
			f.seek(0, 0)
			print f.readline(),
		else:
			print "\nMatch NOT found in the following tale"
			print f.readline(),
...
def all_occurence(qstring, fn_list, stree):
	for i in range(1,314):                
		...
		if result is 1:
			#print "Match found in the following tale at index", index
...
def relevance(qstring, fn_list, stree):
	...
	for i in range(top):
		print sorted_sc[i][2],'=>', sorted_sc[i][0], '=>', sorted_sc[i][1]
